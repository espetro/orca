...
A host is whichever Orca install you paired from — your laptop's desktop, an `orca serve` on a server, or both. Each is an independent host identity; the mobile holds a separate device token per host and reaches each one on its own connection. Turning off the laptop does not affect sessions owned by `orca serve`, and vice versa. The mobile lists worktrees from every paired host in one sidebar, but agent traffic and terminal PTYs always stay on the host that owns them.

See [Cross-instance host architecture](/docs/reference/cross-instance-host-architecture) for the model underneath (the Orca Relay, transport selection, and which state lives where).
