...
} from './verify-localization-catalog.mts'
...
describe('verify-localization-catalog', () => {
  it('bootstraps English entries without fabricating target translations', async () => {
    ...
    await expect(verifyLocalizationCatalog(root, { fix: false, pluginCatalogs: [] })).resolves.toBe(
      1
    )
  ...
  it('never overwrites mismatched translations or removes target-only entries', async () => {
    ...
    await expect(verifyLocalizationCatalog(root, { fix: true, pluginCatalogs: [] })).resolves.toBe(
      1
    )
