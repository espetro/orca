...
import { collectLocalizationCandidates } from './audit-localization-coverage.mts'
  ...
  const replacements = uniqueCandidates(candidates)
    ...
    .filter(
      (
        ...
        translation: NonNullable<(typeof candidates)[number]>
      } => entry.translation !== null
    )
    .sort((left, right) => (right.candidate.start as number) - (left.candidate.start as number))
  ...
  while (stack.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (!['.git', 'assets', 'dist', 'node_modules', 'out', '__snapshots__'].includes(name)) {
          stack.push(fullPath)
        }
        continue
      }
      if (
        entry.isFile() &&
        /\.(?:ts|tsx|js|jsx|mts|cts)$/.test(name) &&
        !name.endsWith('.d.ts') &&
        !name.includes('.test.') &&
        !name.includes('.spec.')
      ) {
