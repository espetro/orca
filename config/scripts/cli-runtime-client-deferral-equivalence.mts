...
function run(entry: string, argv: string[], env: Record<string, string> = {}) {
...
const UNSAFE_TOKENS = new Map<string, string>(
  FOREGROUND_TOKENS.flatMap(
    (token) => [[token, 'runs in the foreground or spawns a process']] as [string, string][]
  ).concat(
    MUTATING_TOKENS.flatMap(
      (token) => [[token, 'can write outside ORCA_USER_DATA_PATH']] as [string, string][]
    )
  )
)
