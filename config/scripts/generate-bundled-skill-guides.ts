...
async function assertStubSourcesMatchTopics(repoRoot: string) {
...
async function buildArtifacts(repoRoot = REPO_ROOT) {
  ...
  const guides: { name: string; description: string; markdown: string; aliases: string[] }[] = []
  const projections: { path: string; content: string }[] = []
