...
export function parseSignatureJson(stdout) {
  ...
  try {
    ...
    throw new Error(`PowerShell returned malformed signature JSON: ${message}`)
  }
}

export function classifySignature(
  signature: Record<string, unknown>,
  options: { expectedSigners?: string[]; expectedThumbprints?: string[] } = {}
) {
...
type SpawnSyncLike = (command: string, args: string[], options: object) => SpawnSyncResult
...
export function getPowerShellSignatureJson(
  ...
  const stderrText = typeof result.stderr === 'string' ? result.stderr : result.stderr?.toString()
  if (stderrText?.trim()) {
