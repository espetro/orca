...
const dailyWorkflow = parseWorkflow(
  readFileSync(join(projectDir, '.github/workflows/daily-mac-build.yml'), 'utf8')
)
const e2eWorkflow = parseWorkflow(
  readFileSync(join(projectDir, '.github/workflows/e2e.yml'), 'utf8')
)
...
describe('daily E2E dispatch contract', () => {
  it('dispatches SHA-scoped full E2E only after a live daily publish', () => {
    ...
    if (dispatchStep === undefined) {
      throw new Error('dispatch step missing')
    }
