...
function assertSuccessfulWrite(result: ShellResult, label: string) {
...
function findStrings(value: unknown, matches: string[] = []): string[] {
...
function nextRequest(server: ReturnType<typeof createServer>): Promise<HttpRequest> {
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
    ...
    const onRequest = (
      ...
      response: { writeHead: (code: number, headers: object) => void; end: (data: string) => void }
    ) => {
      const chunks: Buffer[] = []
...
async function verifyNoOpWrites(
  scripts: { fileName: string; path: string }[],
  home: string,
  payload: string
) {
...
async function verifyClaudeDevinSkip(
  scripts: { fileName: string; path: string }[],
  home: string,
  payload: string
) {
  ...
  if (!claude) {
    throw new Error('claude-hook.sh not found in managed scripts')
  }
...
async function verifyForwarding(
  scripts: { fileName: string; path: string; source: string }[],
  home: string,
  payload: string
) {
  ...
  await new Promise<void>((resolve, reject) => {
    server.once('error', reject)
    server.listen({ port: 0, hostname: '127.0.0.1' }, resolve)
  })
  try {
    const address = server.address() as { port: number } | null
    if (!address) {
