...
function runElectronPackageBinaryInstall() {
  ...
  const result = spawnSync(
    process.execPath,
    ['config/scripts/install-electron-package-binary.ts'],
    {
      cwd: projectDir,
      env,
      stdio: 'inherit'
  ...
  if (result.status !== 0) {
    throw new Error(
      `config/scripts/install-electron-package-binary.ts exited with status ${result.status}`
    )
  }
}
...
function continuePostinstallWithoutElectron() {
  ...
  console.error(
    '[rebuild] Continuing postinstall because Electron binary installation failed. ' +
      ...
      'config/scripts/ensure-native-runtime.ts --runtime=electron before launching Electron.'
  )
  return true
}
