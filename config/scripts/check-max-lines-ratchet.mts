...
export function collectMobileBumps(configText: string): string[] {
  const cfg = JSON.parse(configText) as {
    overrides?: { rules?: Record<string, unknown>; files?: string[] }[]
  }
  const bumps: string[] = []
...
export function main(root = process.cwd()) {
  ...
  if (!fs.existsSync(baselineFile)) {
    console.error(
      `::error::Missing ${BASELINE_PATH}. Generate it with: node config/scripts/check-max-lines-ratchet.mts --init`
    )
    return 1
  }
