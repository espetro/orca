...
} from './verify-windows-inner-signature.mts'
...
describe('verify-windows-inner-signature', () => {
  ...
  it('runs PowerShell with an argument array and fails on stderr or nonzero exit', () => {
    ...
    const spawnSyncImpl = (command: string, args: string[], options: object) => {
      calls.push({ command, args, options })
      return { status: 0, stdout: JSON.stringify(validSignature), stderr: '' } as const
    }
    ...
    expect(() =>
      getPowerShellSignatureJson(
        'Orca.exe',
        (_c, _a, _o) =>
          ({ status: 0, stdout: '{}', stderr: 'warning' }) as ReturnType<typeof spawnSync>
      )
    ).toThrow(/stderr/)
    expect(() =>
      getPowerShellSignatureJson(
        'Orca.exe',
        (_c, _a, _o) => ({ status: 7, stdout: '', stderr: '' }) as ReturnType<typeof spawnSync>
      )
    ).toThrow(/exit code 7/)
  })
  ...
  it('verifies with injected Windows platform and spawn implementation', () => {
    withTempFile((filePath) => {
      const signature = verifyWindowsInnerSignature({
        ...
        platform: 'win32',
        spawnSyncImpl: ((_p: string, _a: string[], _o: object) => ({
          status: 0,
