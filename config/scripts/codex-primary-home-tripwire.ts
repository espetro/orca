...
export async function snapshotCodexHome(codexHome: string) {
...
type HomeEntry = { path: string; type: string }
type HomeSnapshot = { codexHome: string; digest: string; entryCount: number; entries: HomeEntry[] }

type HomeChangeEvent = {
  beforeDigest: string
  afterDigest: string
  changedPaths: string[]
  detectedAt?: string
...
export function describeCodexHomeChange(
  before: HomeSnapshot,
  after: HomeSnapshot
): HomeChangeEvent | null {
...
export async function startCodexPrimaryHomeTripwire(
  options: {
    primaryHome?: string
    intervalMs?: number
    onChange?: (event: HomeChangeEvent & { detectedAt?: string }) => void
  } = {}
) {
  ...
  const scan = async () => {
    ...
    try {
      ...
      const event: HomeChangeEvent = {
        beforeDigest: currentSnapshot.digest,
        afterDigest: currentSnapshot.digest,
        changedPaths: [],
