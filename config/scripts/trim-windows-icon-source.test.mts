...
import {
  ...
  resizeImage,
  squareWithMargin
} from './trim-windows-icon-source.mts'
...
function largestIcoFrame(
  icoBuffer: Buffer
): { width: number; byteLength: number; imageOffset: number } | null {
  const count = icoBuffer.readUInt16LE(4)
  let best: { width: number; byteLength: number; imageOffset: number } | null = null
...
function decodeIcoFrame(
  icoBuffer: Buffer,
  frame: { width: number; byteLength: number; imageOffset: number }
) {
...
describe('Windows ICO glyph fill', () => {
  it('committed resources/build/icon.ico fills the canvas', () => {
    ...
    const frame = largestIcoFrame(ico)
    if (!frame) {
      throw new Error('ICO has no frames')
    }
    const decoded = decodeIcoFrame(ico, frame)
    expect(glyphFillFraction(decoded)).toBeGreaterThanOrEqual(MIN_GLYPH_FILL_FRACTION)
  })
...
describe('trim pipeline', () => {
  ...
  it('crops to the glyph then re-squares with a small margin', () => {
    ...
    const bounds = findOpaqueBounds(image)
    if (!bounds) {
      throw new Error('image should not be fully transparent')
    }
  ...
  it('downscales without leaving the glyph undersized', () => {
    ...
    const imageBounds = findOpaqueBounds(image)
    if (!imageBounds) {
      throw new Error('image should not be fully transparent')
    }
    const filled = squareWithMargin(cropImage(image, imageBounds))
