...
import { collectRendererDiagnostics } from './linux-wayland-renderer-diagnostics.ts'
import {
  assertKeyboardInputWorks,
  assertScrollbackBufferWorks,
  setupTerminal
} from './linux-wayland-terminal-exercise.ts'
import {
  createPhaseLogger,
  runWithTimeout,
  startValidationWatchdog
} from './linux-wayland-validation-watchdog.ts'
...
function ensureElectronRuntime() {
  run(process.execPath, ['config/scripts/ensure-native-runtime.ts', '--runtime=electron'])
}
...
async function runValidation(mode) {
  ...
  let commandLineSwitches: any = null
