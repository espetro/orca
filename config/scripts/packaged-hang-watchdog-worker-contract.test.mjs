...
describe('packaged hang watchdog worker contract', () => {
  it('boots the worker from app.asar in PR checks', () => {
    ...
    const smokeSource = readFileSync(
      'config/scripts/smoke-packaged-hang-watchdog-worker.ts',
      'utf8'
    )
    ...
    expect(smokeStep.run).toBe(
      'xvfb-run --auto-servernum node config/scripts/smoke-packaged-hang-watchdog-worker.ts --app-dir=dist/linux-unpacked'
    )
