...
function listTsFiles(dir) {
  ...
  return fs
    .readdirSync(absolute, { recursive: true, withFileTypes: false })
    .map((entry) => path.join(dir, String(entry)))
    .filter(
      (file) => /\.tsx?$/.test(file) && !/\.test\.tsx?$/.test(file) && !/test-harness/.test(file)
    )
}
...
function collectChannelConstants(files) {
  ...
  for (const file of files) {
    ...
    for (const match of source.matchAll(
      /(?:const|let)\s+(\w+)\s*=\s*'([a-zA-Z][\w-]*:[\w:-]+)'/g
    )) {
  ...
  for (const match of source.matchAll(callPattern)) {
    ...
    if (channel) {
      sites.push({
        ...
        file,
        line,
        kind: /** @type {PreloadCallKind | MainRegistrationKind} */ (kind)
      })
    }
  }
  return sites
...
const MAIN_PUSH_NAMED_HELPER_PATTERN =
  /\b(?:sendToTrustedUIRenderer|sendToRenderer)\(\s*'([a-zA-Z][\w-]*:[\w:-]+)'/g
...
export function scanChannelSites() {
  ...
  const constants = collectChannelConstants([
    ...preloadFiles,
    ...mainFiles,
    ...listTsFiles(SHARED_DIR)
  ])
    ...
    for (const [method, kind] of PRELOAD_CALLS) {
      preloadSites.push(
        ...extractCallSites(
          file,
          source,
          new RegExp(`ipcRenderer\\.${method}\\(\\s*([^)]*)`, 'g'),
          kind,
          constants
        )
      )
  ...
  for (const file of mainFiles) {
    ...
    mainSites.push(
      ...extractSendHelperSites(file, source, MAIN_PUSH_QUALIFIED_CONST_PATTERN, constants)
    )
    mainSites.push(
      ...extractSendHelperSites(file, source, MAIN_PUSH_NAMED_HELPER_PATTERN, constants)
    )
  }
...
export function runParityCheck() {
  ...
  console.log(
    `ipc channel parity: preload-only violations ${report.preloadOnly.length}, main-only violations ${report.mainOnly.length}`
  )
