...
import type { SkillFileEntry } from './skill-bundle-git-ops.ts'
import {
  appendReleaseRow,
  ...
  sortManifestFiles
} from './generate-skill-bundle-manifest.ts'
import { classifyFile, normalizeText } from './skill-bundle-git-ops.ts'
...
async function createReleaseSandbox() {
  ...
  const script = path.join(root, 'config', 'scripts', 'generate-skill-bundle-manifest.ts')
  await mkdir(path.dirname(script), { recursive: true })
  await mkdir(skillRoot, { recursive: true })
  await copyFile(path.join(import.meta.dirname, 'generate-skill-bundle-manifest.ts'), script)
  await copyFile(
    path.join(import.meta.dirname, 'skill-bundle-git-ops.ts'),
    path.join(root, 'config', 'scripts', 'skill-bundle-git-ops.ts')
  )
  await copyFile(
    path.join(import.meta.dirname, 'skill-bundle-manifest-types.ts'),
    path.join(root, 'config', 'scripts', 'skill-bundle-manifest-types.ts')
  )
...
describe('skill bundle manifest generator', () => {
  ...
  it('rejects rewrites of released snapshots and allows floating-tail replacement', () => {
    ...
    expect(() => assertReleasedHistoryPreserved(null, artifacts, null)).not.toThrow()
  })
