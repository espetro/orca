...
export function encodePng(frame: { width: number; height: number; data: Buffer }): Buffer {
...
export function findOpaqueBounds(
  { width, height, data }: { width: number; height: number; data: Buffer },
  threshold = ALPHA_BACKGROUND_THRESHOLD
) {
  let minX = width
  let minY = height
  let maxX = -1
...
export function cropImage(
  { width, data }: { width: number; data: Buffer },
...
export function squareWithMargin(
  image: { width: number; height: number; data: Buffer },
  marginRatio = ICON_CANVAS_MARGIN_RATIO
) {
...
export function encodeIco(frames: { width: number; height: number; data: Buffer }[]) {
