...
import { INTL_AI_LOCALES_DIR, buildIntlAiConfig, INTL_AI_TARGET_LOCALES } from './intl-ai.config.ts'
import type { LocaleCatalog } from './locale-translation-policy.ts'
import { resolveOpenRouterApiKey } from './intl-ai-env.ts'
import { resolveModelPolicy } from './intl-ai-model-policy.ts'
import { runPostFillGate, writeReport } from './intl-ai-post-fill-gate.ts'
...
async function loadStashKey(): Promise<{ value: string }> {
  ...
  const { ORCA_STASH_ENV_PATH } = await import('./intl-ai-env.ts')
...
if (process.argv[1]?.endsWith('intl-ai-translate-locales.ts')) {
  main().then((code) => {
