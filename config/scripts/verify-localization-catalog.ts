...
import { canonicalGenericRenderings } from './locale-overrides.ts'
import { repairTranslatedValue } from './locale-translation-policy.ts'
...
function collectInterpolationVariables(value: unknown): string[] {
...
export function collectGenericTermRegressions(enEntries, localeEntries, localeName) {
  ...
  const regressions: { key: string; form: string; localeValue: string; repaired: string }[] = []
  ...
  if (
    ...
    if (genericTermRegressions.length > 0) {
      ...
      console.error(
        ...
        'Treat the term as generic in config/scripts/locale-generic-ui-terms.ts',
...
function parseArgs(argv: string[]): { fix: boolean; pluginCatalogs: string[] } {
  const pluginCatalogs: string[] = []
...
export async function main(
  ...
  options: { fix?: boolean; pluginCatalogs?: string[]; [key: string]: any } = parseArgs(
