 ...
 */
export function readLockfileResolutionHashes(lockfileText: string, packageKey: string): string[] {
  return Array.from(
...
function run(command, args, options = {}): string {
...
function fetchPristinePackage(packageEntry, workDir) {
  ...
  const tarballLines = output.trim().split('\n')
  const tarball = path.join(target, tarballLines.at(-1)!.trim())
...
function diffFolders(folderA, folderB) {
  ...
  try {
    ...
    const err = error as { status?: number; stdout?: unknown; stderr?: string | undefined }
    if (err.status !== 1 || (err.stderr?.length ?? 0) > 0) {
      throw error
    }
...
const USAGE =
  'Usage: regenerate-xterm-patches.ts [--check | --write] [--work-dir=<path>]\n' +
