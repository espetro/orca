...
import { resolvePullRequestDiffBase } from './git-pull-request-diff-base.ts'
...
export function overlapsAddedLines(
  startLine: number,
  endLine: number,
  ranges: LineRange[]
): boolean {
...
export function collectAddedLineRanges(
  root: string,
  requestedBase?: string
): { base: string; comparisonBase: string; rangesByFile: Map<string, LineRange[]> } {
...
export function collectBaseLineBlocks(
  root: string,
...
export function diagnosticTouchesAddedLines(
  diagnostic: {
    ...
    line?: number
    filename?: string
    labels?: { span: { start: number; end: number } }[]
  },
