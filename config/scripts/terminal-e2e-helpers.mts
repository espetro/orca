...
export class OrcaTerminal {
  ...
  constructor(public cdpPort: number) {}

  /** Verify connection by taking a snapshot. */
  connect() {
    ab(this.cdpPort, ['snapshot', '-i'])
  }
  ...
  discoverActivePtyId(maxId = 10) {
    ...
    `
    evalInRenderer(this.cdpPort, js)
    ...
    `
    const bufferResult = evalInRenderer(this.cdpPort, bufferJs)
  ...
  probePtyIdWithScreenshot(maxId = 10, screenshotPath = tempScreenshotPath('orca-pty-probe.png')) {
    for (let i = 1; i <= maxId; i++) {
      evalInRenderer(this.cdpPort, `window.api.pty.write('${i}', '\\x03\\x15echo PTY_ID_${i}\\r')`)
    }
    sleep(2_000)
  ...
  send(ptyId, text) {
    ...
    evalInRenderer(this.cdpPort, `window.api.pty.write('${ptyId}', '${escaped}')`)
  }
  ...
  screenshot(path = tempScreenshotPath('orca-terminal.png')) {
    ab(this.cdpPort, ['screenshot', path])
    return path
  }
   ...
   */
  newTerminal() {
    ab(this.cdpPort, ['click', '@e7']) // "New terminal (Cmd+T)" button
    sleep(2_000)
  }
