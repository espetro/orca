...
import { runWithTimeout } from './linux-wayland-validation-watchdog.ts'
import type { OrcaState } from './renderer-globals.ts'
import './renderer-globals.ts'
...
async function getTerminalContent(page, charLimit = 12_000) {
  return page.evaluate((limit) => {
    ...
    const store = w.__store as { getState(): Record<string, unknown> } | undefined
    if (!store || !w.__paneManagers) {
      return ''
    }
    ...
    const tabId =
      state?.activeTabType === 'terminal'
        ...
        : worktreeId && worktreeId !== ''
          ...
          : null
    const manager = tabId ? w.__paneManagers.get(tabId) : null
    const pane = (manager?.getActivePane?.() ?? manager?.getPanes?.()[0] ?? null) as unknown as {
  ...
  await runWithTimeout(
    ...
    () =>
      page.evaluate(() => {
        ...
        const state: OrcaState | undefined = store?.getState() as OrcaState | undefined
        const worktreeId = state?.activeWorktreeId
        ...
        pane.terminal.focus()
        ;(
          pane.container.querySelector('.xterm-helper-textarea') as HTMLInputElement | null
        )?.focus()
      }),
    rendererActionTimeoutMs
  )
...
export async function setupTerminal(page, repoPath, logPhase) {
  ...
  const repoId = await runWithTimeout(
    ...
    () =>
      page.evaluate(async (pathToAdd) => {
        const w = window as unknown as Window
        const result = await w.api?.repos?.add({ path: pathToAdd, kind: 'git' })
        if (!result || 'error' in result || !result.repo) {
          throw new Error(
            result && 'error' in result
              ? (result as { error: string }).error
              : 'repos.add returned null'
          )
        }
        const store = w.__store
        if (!store) {
  ...
  await waitFor('active terminal workspace setup', () =>
    page.evaluate((id) => {
      ...
      const worktree = (state as unknown as { worktreesByRepo?: Record<string, unknown[]> })
        .worktreesByRepo?.[id]?.[0]
      if (!worktree) {
        return false
      }
      state.setActiveWorktree((worktree as unknown as { id: string }).id)
      state = store.getState()
      const tabs = state.tabsByWorktree[(worktree as unknown as { id: string }).id] ?? []
      const tab =
        tabs[0] ??
        state.createTab((worktree as unknown as { id: string }).id, undefined, undefined, {
          activate: true,
      ...
      state = store.getState()
      if (
        state.activeWorktreeId !== (worktree as unknown as { id: string }).id ||
        state.activeTabType !== 'terminal' ||
  ...
  await runWithTimeout(
    ...
    () =>
      page.evaluate(() => {
        ...
        if (!terminal) {
          throw new Error('No terminal on pane.')
        }
