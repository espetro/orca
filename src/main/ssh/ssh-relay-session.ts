...
import { SshRelayPtyRecovery, isSourceRecoveryCancellationError } from './ssh-relay-pty-recovery'
...
export class SshRelaySession {
  ...
  private readonly ptyRecovery: SshRelayPtyRecovery
  ...
  constructor(
    ...
    this.ptyRecovery = new SshRelayPtyRecovery({
      targetId,
      store,
      runtime: this.runtime,
      getMainWindow: () => this.getMainWindow(),
      mux: () => this.mux,
      activePtyProviderGeneration: () => this.activePtyProviderGeneration,
      activePtyConsumerOwner: () => this.activePtyConsumerOwner()
    })
  }
  ...
  async establish(conn: SshConnection, graceTimeSeconds?: number): Promise<void> {
    ...
    try {
      ...
      await this.ptyRecovery.reattachKnownPtys(mux, shouldContinue)
  ...
  private async registerProviders(
    ...
    if (consumerOwnerState) {
      ptyProvider.setPtyDeliveryPauseAdapter?.(({ id, providerGeneration: generation, paused }) => {
        ...
        const sourceIdentity = this.ptyRecovery.sourceIdentityFor(id)
    ...
    this.ptyRecovery.installPtyRecoveryNotifications(mux)
  ...
  private teardownProviders(
    ...
    if (this.activePtyProviderGeneration !== null) {
      ...
      if (reason === 'connection_lost' && this.activePtyConsumerOwner()?.outputFlowControl) {
        this.ptyRecovery.beginPtyModelMigration(providerGeneration, outputGenerationReason)
      } else {
    ...
    this.ptyRecovery.disposeForTeardown()
  }
  ...
  private wireUpPtyEvents(
    ...
    ptyProvider.onData((payload) => {
      ...
      const pending = this.ptyRecovery.pendingReattachFor(payload.id)
      if (pending && this.activePtyConsumerOwner()?.outputFlowControl) {
        if (pending.livePassthrough) {
          void this.ptyRecovery.acceptPtyData(payload).catch(() => {})
          return
        }
        this.ptyRecovery.quarantineReattachData(pending, payload)
        return
      }
      void this.ptyRecovery.acceptPtyData(payload).catch(() => {})
    })
    ptyProvider.onRejectedData?.((payload) => {
      ...
      if (pending) {
        ...
        this.ptyRecovery.wakeRecovery(pending)
        return
      }
      void this.ptyRecovery.acceptPtyData(payload).catch(() => {})
    })
    ...
    ptyProvider.onExit((payload) => {
      ...
      const pendingReattach = this.ptyRecovery.pendingReattachFor(payload.id)
      if (pendingReattach && !pendingReattach.activated) {
        ...
        this.ptyRecovery.wakeRecovery(pendingReattach)
        return
      }
      if (!isCurrentPtyExit(payload)) {
        return
      }
      void this.ptyRecovery.acceptPtyExit(payload).catch(() => {})
    })
  }
}
