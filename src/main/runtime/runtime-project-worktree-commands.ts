...
import { RuntimeProjectWorktreeProjectWorkspaceCommands } from './runtime-project-worktree-project-workspace-commands'
import { RuntimeProjectWorktreeRepoCrudCommands } from './runtime-project-worktree-repo-crud-commands'
...
export class RuntimeProjectWorktreeCommands {
  ...
  constructor(deps: RuntimeProjectWorktreeCommandsDeps) {
    this.deps = deps
    this.projectWorkspaceCommands = new RuntimeProjectWorktreeProjectWorkspaceCommands(deps)
  ...
  async setupProjectExistingFolder(
    ...
    return self.completeProjectHostSetup(args, repo, !knownRepoIds.has(repo.id))
  }
  ...
  private linkRepoToProjectHostSetup(
    ...
    let setup = getProjectHostSetupForRepo(self.deps.listProjectHostSetups(), repo)
    if (setup.projectId !== args.projectId) {
      const existingProject = self.listProjects().find((project) => project.id === args.projectId)
    ...
    repo = updated
    setup = getProjectHostSetupForRepo(self.deps.listProjectHostSetups(), repo)
    const project = self.listProjects().find((entry) => entry.id === setup.projectId)
    if (!project) {
