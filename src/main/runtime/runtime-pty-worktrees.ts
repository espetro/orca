...
export class RuntimePtyWorktrees {
  ...
  getMobileSessionWorktreeIdsForPty(ptyId: string): string[] {
    return this.deps.managedWorktrees().getMobileSessionWorktreeIdsForPty(ptyId)
  }

  getMobileTerminalLeafPtyIds(tab: RuntimeMobileSessionTerminalTab): string[] {
    return this.deps.mobileTabSnapshots().getMobileTerminalLeafPtyIds(tab)
  }
  ...
  async inspectTerminalProcessIncarnationLiveness(
    ...
    if (!hostScope || !this.deps.ptyController()?.listProcesses) {
      return 'unverifiable'
    }
    const listed = await withTimeoutResult(
      this.deps.ptyController().listProcesses(hostScope.kind === 'ssh' ? hostScope.targetId : null),
      PTY_CONTROLLER_LIST_TIMEOUT_MS
    )
    if (!listed.ok) {
      return 'unverifiable'
  ...
  onPtyData(
    ...
    try {
      ...
      try {
        ...
        this.deps.flushPendingTerminalSideEffectFacts(ptyId, titleTrackerEntry)
      }
    }
  ...
  ptyExit_decideSshSurface(
    ...
    const incarnationId =
      ...
      (`runtime:${this.deps.runtimeId()}:${this.getPtyLifecycleGeneration(ptyId)}` as PtyIncarnationId)
  ...
  async stopExplicitlyClosedTabPtys(
    ...
    for (const ptyId of ptyIds) {
      ...
      if (this.deps.ptyController()?.stopAndWait) {
        try {
          stopped = await this.deps.ptyController().stopAndWait(ptyId, { deadlineMs })
        } catch (error) {
