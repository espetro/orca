...
import {
  AgentStatusInferenceRegistry,
  type AgentStatusInferenceDeps
} from './agent-status-inference'
...
export class AgentHookServer {
  ...
  private readonly inference: AgentStatusInferenceRegistry
  ...
  constructor() {
    ...
    this.inference = new AgentStatusInferenceRegistry(this as unknown as AgentStatusInferenceDeps)
  }
  ...
  private withdrawReplayObservation(paneKey: string): void {
    if (this.runtimeObservedStatusPaneKeys.delete(paneKey)) {
      this.inference.notifyStatusChangeListeners()
    }
  }
  ...
  getProviderSessionIdentities(): AgentHookProviderSessionIdentity[] {
    return this.inference.buildStatusChangeNotification().providerSessions
  }
  ...
  inferInterrupt(request: AgentInterruptInferenceRequest): boolean {
    return this.inference.inferInterrupt(request)
  }
  ...
  inferQuestionAnswered(request: AgentQuestionAnsweredInferenceRequest): boolean {
    return this.inference.inferQuestionAnswered(request)
  }

  getStatusChangeSnapshot(): AgentHookStatusChangeEntry[] {
    return this.inference.buildStatusChangeNotification().statuses
  }
