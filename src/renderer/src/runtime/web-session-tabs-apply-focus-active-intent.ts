...
import { buildHostToLocalTabIdMap } from './web-session-tabs-mirrored-groups'
import { updateHostSessionTabIdMappings } from './web-session-tabs-tab-id-mapping'
...
// Active/intent resolution stage (extracted from web-session-tabs-apply-focus).
export function buildWebSessionTabsActiveIntentStage(
  state: WebSessionTabsSyncState,
  environmentId: string,
  mirrored: NonNullable<WebSessionTabsMirroredContext>,
  focus: {
    nextUnifiedTabs: Tab[] | null
    retainedUnifiedTabs: Tab[]
  ...
  const { nextUnifiedTabs, mirroredUnifiedTabs } = focus as {
    nextUnifiedTabs: Tab[] | null
    mirroredUnifiedTabs: Tab[]
  }
  ...
  const nextActiveUnifiedTabId =
    ...
    (snapshot.activeTabType === 'agent-session'
      ? (activeMirroredAgentTabId ??
        mirroredAgentTabs.map((e) => e.unifiedTab)[0]?.id ??
